...
ffbuild_dockerdl() {
    ...
    echo "cd libplacebo && git submodule update --init --recursive --depth=1 --filter=blob:none"
    echo "git-mini-clone \"$SCRIPT_REPO2\" \"$SCRIPT_COMMIT2\" libplacebo2"
    echo "cd libplacebo2 && git submodule update --init --recursive --depth=1 --filter=blob:none"
}
